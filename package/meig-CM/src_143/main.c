...
static int usage(const char *progname) {
     ...
     dbg_time("-d device                              QMI device");
	 dbg_time("[Examples]");
     dbg_time("Example 1: %s ", progname);
     dbg_time("Example 2: %s -s 3gnet ", progname);
     dbg_time("Example 3: %s -s 3gnet -i usb0 -d /dev/qcqmi0", progname);
     dbg_time("Example 4: %s -s 3gnet carl 1234 0 -p 1234 -f gobinet_log.txt", progname);
     return 0;
}
