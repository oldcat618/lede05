...
static int usage(const char *progname) {
	 ...
	 dbg_time("-k channelID                           Send SIGINT to meig-CM which multi-pdn is channelID.");
...
__main_loop:
    ...
    dbg_time("qmap mod is:%d,qmap version is:%d",profile->qmap_mode,profile->qmap_version);
...
#endif
    dbg_time("befor enter setethmode qmap version is:%d",profile->qmap_version);
...
int main(int argc, char *argv[])
{
    ...
    dbg_time("Meig_QConnectManager_Linux_V1.4.4");
    ...
    dbg_time("main function before run ql_qmap_mode_detect qmap  version is:%d,qmap mode is:%d",profile.qmap_version,profile.qmap_mode); 
    ql_qmap_mode_detect(&profile);
    dbg_time("after function qmap version is:%d,qmap mode is:%d",profile.qmap_version,profile.qmap_mode);
