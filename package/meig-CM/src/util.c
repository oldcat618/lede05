...
void ql_get_driver_rmnet_info(PROFILE_T *profile, RMNET_INFO *rmnet_info) {
    ...
    dbg_time("[ql_get_drier_rmnet_info] zhangqingyun add get qmap settings\n");
    ...
    dbg_time("usnet_adapter name is:%s",profile->usbnet_adapter);
    ...
    if (rc < 0) {
        ...
        dbg_time("rmnet_Info rx_urb_size is:%d,qmap_mode is:%d,qmap_version is:%d,inface id is:%d",rmnet_info->rx_urb_size,rmnet_info->qmap_mode,
        rmnet_info->qmap_version,rmnet_info->iface_id);
    }
...
void ql_set_driver_qmap_setting(PROFILE_T *profile, QMAP_SETTING *qmap_settings) {
    ...
    dbg_time("zhangqingyun add qmap settings set \n");
    memset(&ifr, 0, sizeof(struct ifreq));
    strncpy(ifr.ifr_name, profile->usbnet_adapter, IFNAMSIZ);
    ifr.ifr_name[IFNAMSIZ - 1] = 0;    
