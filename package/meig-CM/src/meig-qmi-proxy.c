...
#define MEIG_UL_DATA_AGG
//#define MEIG_QMI_MERGE
...
static void ql_get_driver_rmnet_info(char *ifname, RMNET_INFO *rmnet_info) {
    ...
    if (rc < 0) {
        ...
        dprintf("[ql_get_driver_rmnet_info rx_urb_size is:%d,iface_id is:%d,qmap_mode is:%d,qmap_version is:%d]",rmnet_info->rx_urb_size,
        rmnet_info->iface_id,rmnet_info->qmap_mode,rmnet_info->qmap_version);
    }
...
static int qmidevice_detect(char *device_name, char *ifname) {
    ...
    if (pDir)  {
        ...
        while ((ent = readdir(pDir)) != NULL)  {
            ...
            if (strncasecmp(idVendor, "05c6", 4) && strncasecmp(idVendor, "2dee", 4))
                continue;
            ...
            if (access(pl->subdir, R_OK)) {
                ...
                if (access(pl->subdir, R_OK)) {
                    dprintf("no GobiQMI/usbmic/usb found in %s/%s:1.5\n", dir, ent->d_name);
                    continue;
                }
            }
            ...
            snprintf(pl->subdir, sizeof(pl->subdir), "%s/%s:1.5net", dir, ent->d_name);
...
int main(int argc, char *argv[]) {
   ...
   if (cdc_wdm[0] == '\0' || ifname[0] == '\0') {
    ...
    sprintf(servername, "meig-qmi-proxy%d", cdc_wdm[strlen(cdc_wdm)-1]-'0');
