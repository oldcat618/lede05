/******************************************************************************
  ...
  DESCRIPTION
  Connectivity Management Tool for USB network adapter of Meig wireless cellular modules.
...
int requestSetEthMode(PROFILE_T *profile) {
    ...
    if (profile->qmap_mode) {
        ...
        //    dbg_time("pcie zhangqingyun interface id is:%d",qmap_settings.iface_id);
        }
        else { // for MDM9x07&MDM9x40&SDX20 USB
            qmap_settings.rx_urb_size = profile->qmap_size; //SDX24&SDX55 support 32KB
            qmap_settings.ep_type = DATA_EP_TYPE_HSUSB;
            qmap_settings.iface_id = 0x05;
            dbg_time("usb zhangqingyun interface id is:%d",qmap_settings.iface_id);
        }
        ...
        dbg_time("qmap_verion is:%d",profile->qmap_version);
        if(profile->qmap_version != 0x09)
            profile->qmap_version = 0x05;

        qmap_version = profile->qmap_version;
        dbg_time("qmap_settins rmnet_info_size = %d,iface_id = %d\n",profile->rmnet_info.size,profile->rmnet_info.iface_id);
        #if 1
        if (profile->rmnet_info.size) {
            ...
            dbg_time("requestSetEtheMode rx_urb_size is:%d,iface_id is:%d,qmap_version is:%d",profile->rmnet_info.rx_urb_size,profile->rmnet_info.iface_id,profile->rmnet_info.qmap_version);
        }
        #endif
    ...
    dbg_time("send qmi data format \n");
    err = QmiThreadSendQMI(pRequest, &pResponse);
    //dbg_time("zhangqingyun add mux error 1?\n");
    //sleep(10);
    qmi_rsp_check_and_return();
   // dbg_time("zhangqingyun add mux error 2?");
