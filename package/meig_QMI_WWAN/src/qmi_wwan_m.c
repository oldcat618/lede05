...
#define MEIG_WWAN_VERSION "Meig_Linux&Android_QMI_WWAN_Driver_"VERSION_NUMBER
static const char driver_name[] = "qmi_wwan_m";
...
*/
#define MEIG_WWAN_QMAP 4//MAX is 7
...
#define MEIG_BRIDGE_MODE
#endif

#ifdef MEIG_BRIDGE_MODE
static uint __read_mostly bridge_mode = 0/*|BIT(1)*/;
module_param( bridge_mode, uint, S_IRUGO );
#endif

#if defined(MEIG_WWAN_QMAP)
#define MEIG_UL_DATA_AGG 1

#if defined(MEIG_UL_DATA_AGG)
struct tx_agg_ctx {
...
static int bridge_arp_reply(struct net_device *net, struct sk_buff *skb, uint bridge_ipv4) {
    ...
    pr_info("meig smart open bridge mode ?\n");
...
static int qmap_register_device(sQmiWwanQmap * pDev, u8 offset_id)
{
    ...
    priv->mux_id = MEIG_QMAP_MUX_ID + offset_id;
	...
	struct sk_buff *skb_in, struct sk_buff_head *skb_chain)
{
	...
	while (skb_in->len > sizeof(struct qmap_hdr)) {
		...
		mux_id = map_header->mux_id - MEIG_QMAP_MUX_ID;
...
#if defined(MEIG_WWAN_QMAP)// && defined(CONFIG_ANDROID)
...
static int qmi_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (dev->driver_info->flags & FLAG_NOARP) {
		...
		dev_info(&intf->dev, "Meig %s work on RawIP mode\n", buf);
	...
	if (!status)
	{
...
#ifdef MEIG_BRIDGE_MODE
		pQmapDev->bridge_mode = bridge_mode;
                printk("zqy meig bridge mode open \n");
#endif
                printk("zqy driver bind to set qmap\n");
		...
		if (dev->driver_info->flags & FLAG_NOARP)
		{
			...
			int idProduct = le16_to_cpu(dev->udev->descriptor.idProduct);
                        printk("zqy drive bind qmap_version is:%d\n",qmap_version);
			int lte_a = (idProduct == 0x4d22);//zqy insert add sdx65 sdx55 product id is 0x4d22?
			...
			if (lte_a && pQmapDev->qmap_mode == 0) {
				...
				if(qmap_mode == 0)
					qmap_mode = 1; //old meig-CM only check sys/module/wwan0/parameters/qmap_mode
			}
			...
			if (pQmapDev->qmap_mode) {
				...
				else if (idProduct == 0x4d22 || idProduct == 0xf601)
				...
				pQmapDev->rmnet_info.iface_id = 5; //zqy add for meigsmart modules rmnet interface
...
static const struct usb_device_id products[] = {
	{ QMI_FIXED_RAWIP_INTF(0x05C6, 0xf601, 5,mdm9x07) },  /* Meig slm7xx */
        { QMI_FIXED_RAWIP_INTF(0x2DEE, 0x4D22, 5, sdx55) }, /*Meig SRM8xx*/
	{ }					/* END */
};
			  ...
			  const struct usb_device_id *prod)
{
	...
	if (intf->cur_altsetting->desc.bInterfaceClass != 0xff) {
		dev_info(&intf->dev,  "Meig module not qmi_wwan mode! please check 'at+qcfg=\"usbnet\"'\n");
		return -ENODEV;
	}
